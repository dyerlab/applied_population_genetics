...
```{r,message=FALSE,warning=FALSE}
require(leaflet)
leaflet( data=coords ) %>% addTiles() %>% addMarkers( ~Longitude, ~Latitude, popup=~as.character(Stratum))
```
...
leaflet( data=coords ) %>% 
  ...
  addMarkers( ~Longitude, ~Latitude, popup=~as.character(Stratum))
```
...
leaflet( data=coords ) %>%  
  ...
  addCircleMarkers( ~Longitude, ~Latitude, 
                    popup=~as.character(Stratum), 
                    radius=~log(coords$Ae)*10 + 1, color="red", 
                    fillOpacity=0.75) 
```
