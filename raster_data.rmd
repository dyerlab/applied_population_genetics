...
which can be visualized using the normal plot command. The raster library has overrid several of the plotting functions and you can plot raster objects and decorate the images in the same way you do for normal plotting materials (\@ref(stock-graphics)).
...
ggplot( df, aes(x=Elevation,fill=Category)) + geom_histogram(stat="bin") + xlab("Elevation (m)") + ylab("Distribution of Permuted Elevations")
```
